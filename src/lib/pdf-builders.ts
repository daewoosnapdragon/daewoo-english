...
function makePdfHelper(pdf: PDFDocument, font: any, bold: any, italic: any, titleText: string) {
  ...
  function bullet(t: string, sz: number = 8) { checkPage(sz + 6); page.drawText('-', { x: LM + 4, y: y + 1, size: 6, font: bold, color: C.purple }); wrap(t, font, sz, 14); }
...
export async function buildStoryProfilePdf(profile: any, pdf: PDFDocument): Promise<PDFDocument> {
  ...
  if (typeof d !== 'object' || d === null) return pdf;
  const font = await pdf.embedFont(StandardFonts.Helvetica);
  const bold = await pdf.embedFont(StandardFonts.HelveticaBold);
  const italic = await pdf.embedFont(StandardFonts.HelveticaOblique);
  const title = String(d.title || profile.title || 'Story Profile');
  const h = makePdfHelper(pdf, font, bold, italic, title);
  ...
  page.drawText(title, { x: LM, y: 760, size: 20, font: bold, color: C.white });
  if (d.author) page.drawText(`by ${s(d.author)}`, { x: LM, y: 743, size: 10, font: italic, color: rgb(0.85, 0.82, 0.95) });
  y = 720;
  let bx = LM;
  for (const badge of [s(d.genre), s(d.text_structure).replace(/_/g, ' '), s(d.authors_purpose)].filter(Boolean)) {
  ...
  if (d.summary) { y -= 4; push(); h.wrap(s(d.summary), font, 8.5, 4, C.gray); sync(); y -= 8; push(); }
  ...
  h.drawFooter();
    ...
    console.error('Story profile PDF inner error:', pdfErr?.message);
...
export async function buildUnitPlanPdf(plan: any, bookNum: number, moduleNum: number): Promise<PDFDocument> {
  ...
  if (typeof d !== 'object') { const pdf = await PDFDocument.create(); pdf.addPage(); return pdf; }
  ...
  if (d.essential_questions?.length) { h.heading('Essential Questions', C.purple, C.purpleLight); sync(); for (const q of d.essential_questions) { h.checkPage(16); sync(); h.wrap(`Q: ${q}`, bold, 8.5, 4, C.purple); sync(); y -= 2; push(); } }
