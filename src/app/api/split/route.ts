...
export async function POST(request: Request) {
  ...
  try {
    ...
    const insertedIds: string[] = []; // Track for cleanup on failure
    ...
    for (const section of sections) {
      ...
      const hash = crypto.createHash('sha256').update(sectionBuffer).digest('hex').slice(0, 32);
      ...
      if (dbError) {
        ...
        await supabase.storage.from('resources').remove([childPath]);
        uploadedPaths.pop();
        continue;
      }
    ...
    // If nothing was created, don't hide parent — rollback
    if (created.length === 0) {
      return NextResponse.json({ error: 'Split produced no sections. Parent unchanged.' }, { status: 400 });
    }
    ...
    return NextResponse.json({
      ...
      // Rollback: clean up any orphaned uploads and DB records from partial split
      console.error('Split failed mid-operation, rolling back:', splitErr);
      if (uploadedPaths.length > 0) {
        await supabase.storage.from('resources').remove(uploadedPaths).catch(() => {});
      }
      if (insertedIds.length > 0) {
        try { await supabase.from('resources').delete().in('id', insertedIds); } catch {}
      }
      throw splitErr; // Re-throw to outer catch
    }
  } catch (e: any) {
