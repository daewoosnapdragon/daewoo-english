...
import { buildStoryProfilePrompt, cleanJsonResponse } from '@/lib/ai-prompts';
...
- Return ONLY valid JSON.`
    ...
    await supabase.from('story_profiles').delete()
      .eq('resource_id', params.id).eq('user_id', user.id);
    ...
    await supabase.from('resources').update(updates).eq('id', params.id);
