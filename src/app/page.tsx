      ...
      <div className="px-10 py-8">
        <div className="bg-surface border border-border rounded-xl p-12 text-center">
          ...
          <p className="text-text-secondary text-sm max-w-md mx-auto">
            This module is being built. The database tables are ready — we just need to wire up the UI.
          </p>
        </div>
      </div>
    </div>
...
export default function Home() {
  ...
  // Set active semester in context once loaded
  useEffect(() => {
  ...
  const renderView = () => {
    switch (activeView) {
      ...
      case 'tools':
        return <PlaceholderView title={t.nav.tools} description="Grade calculator, score analyzer, and grading scale reference" />
      case 'alerts':
        return <PlaceholderView title={t.nav.alerts} description="View flagged students, generate warning letters" />
      case 'settings':
        return <SettingsView />
      default:
        return <DashboardView />
