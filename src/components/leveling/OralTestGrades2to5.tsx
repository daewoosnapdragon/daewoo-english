...
function SentenceClickableGrid({ sc, studentId, updateScore }: {
  ...
  // Build flat word list with sentence grouping
  const sentenceWords = SENTENCES.map((sent, si) => {
    const words = sent.text.split(/\s+/)
    return words.map((w, wi) => ({ word: w, sentIdx: si, wordIdx: wi, key: `${si}-${wi}` }))
  })
  ...
  const toggle = (sentIdx: number, wordIdx: number) => {
    ...
    setWordStatus(prev => {
      const next = { ...prev, [key]: !prev[key] }
      // Count correct in this sentence
