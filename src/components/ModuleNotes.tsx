...
export default function ModuleNotes({ bookNum, moduleNum, storyTitle, allResources, onSelectResource }: ModuleNotesProps) {
  ...
  const deleteFolder = async (folderId: string) => {
    const ok = await showConfirm('Delete this folder?', { confirmLabel: 'Delete', danger: true });
    if (!ok) return;
    await fetch('/api/module-notes', {
  ...
  return (
    <div className="space-y-1">
      {folders.map(f => (
        <div key={f.id} className="rounded-none border border-sand-100 overflow-hidden">
          <button onClick={() => setOpenFolder(openFolder === f.id ? null : f.id)}
            className="w-full flex items-center gap-2 px-2 py-1.5 hover:bg-cyber-surface text-left">
            <Icon name={ICONS[f.folder_name] || 'folder'} size={14} className="text-cyber-fg" />
            <span className="text-[11px] font-medium text-gray-700 flex-1">{f.folder_name}</span>
            <span className="text-[9px] text-cyber-muted">{(f.resource_ids || []).length}</span>
            <Icon name={openFolder === f.id ? 'expand_less' : 'expand_more'} size={14} className="text-cyber-muted" />
          </button>
            ...
            <div className="border-t border-sand-50 px-2 py-1.5 space-y-1">
              ...
              {editingNotes === f.id ? (
                <div>
                  <textarea value={editText} onChange={e => setEditText(e.target.value)} rows={2} autoFocus
                    className="w-full border border-sand-200 rounded-none px-2 py-1 text-[11px]" placeholder="Teaching notes..." />
                  <div className="flex gap-1 mt-0.5">
                    <button onClick={() => saveNotes(f.id)} className="btn-sweep px-2 py-0.5 text-[9px] font-medium">Save</button>
                    <button onClick={() => setEditingNotes(null)} className="px-2 py-0.5 bg-cyber-surface text-[9px] rounded">Cancel</button>
                  </div>
                </div>
              ) : (
                <div onClick={() => { setEditingNotes(f.id); setEditText(f.notes); }}
                  className="text-[11px] text-cyber-dim cursor-pointer hover:bg-cyber-surface p-1 rounded-none min-h-[20px]">
                  {f.notes || <span className="text-cyber-muted italic">Click to add notes...</span>}
                </div>
              )}
              ...
              {(f.resources || []).map(r => (
                <div key={r.id} className="flex items-center gap-1.5 text-[11px]">
                  <div onClick={() => onSelectResource(r as any)} className="flex items-center gap-1.5 flex-1 cursor-pointer hover:bg-cyber-surface p-0.5 rounded-none truncate">
                    <div className="w-5 h-5 rounded-none bg-cyber-surface flex-shrink-0 flex items-center justify-center overflow-hidden">
                      {r.thumbnail_url ? <img src={r.thumbnail_url} alt="" className="w-full h-full object-cover" /> : <Icon name="description" size={10} className="text-cyber-muted" />}
                    </div>
                    <span className="text-gray-700 truncate">{r.title}</span>
                  </div>
                  <button onClick={() => removeResource(f.id, r.id)} className="p-0.5 text-cyber-muted hover:text-cyber-fg"><Icon name="close" size={12} /></button>
                </div>
              ))}

              {/* Actions */}
              <div className="flex gap-2 pt-0.5">
                <button onClick={() => setSearchForFolder(f.id)} className="text-[9px] text-cyber-fg hover:underline flex items-center gap-0.5">
                  <Icon name="search" size={10} /> Add resource
                </button>
                <button onClick={() => deleteFolder(f.id)} className="text-[9px] text-cyber-dim hover:text-cyber-fg hover:underline">Delete</button>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="flex flex-wrap gap-1 pt-1">
        {PRESET_FOLDERS.filter(n => !folders.some(f => f.folder_name === n)).slice(0, 4).map(n => (
          <button key={n} onClick={() => createFolder(n)}
            className="px-1.5 py-0.5 bg-cyber-surface hover:bg-cyber-surface border border-dashed border-cyber-border rounded-none text-[9px] text-cyber-dim hover:text-cyber-fg flex items-center gap-0.5">
        ...
        {showCustomName ? (
          <div className="flex gap-1 items-center">
            <input value={customName} onChange={e => setCustomName(e.target.value)} placeholder="Name"
              className="border border-sand-200 rounded-none px-1.5 py-0.5 text-[10px] w-20" autoFocus
            ...
            <button onClick={() => { if (customName.trim()) createFolder(customName.trim()); }}
              className="btn-sweep px-1.5 py-0.5 text-[9px] font-medium">Add</button>
