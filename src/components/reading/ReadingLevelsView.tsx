...
export default function ReadingLevelsView() {
  ...
  const [selectedGrade, setSelectedGrade] = useState<Grade>(() => {
    ...
    return (currentTeacher?.grade || 4) as Grade
  })
...
// ─── #44 Cross-Assessment Correlation View ────────────────────────────

function CorrelationView({ students, loading, lang, grade, englishClass }: {
  students: any[]; loading: boolean; lang: string; grade: Grade; englishClass: EnglishClass
}) {
  const [points, setPoints] = useState<{ name: string; korean: string; cwpm: number; avg: number; cls: EnglishClass }[]>([])
  ...
  useEffect(() => {
    (async () => {
      ...
      if (ids.length === 0) { setLoadingData(false); return }
      const [{ data: readings }, { data: grades }] = await Promise.all([
        supabase.from('reading_assessments').select('student_id, cwpm, date').in('student_id', ids).not('cwpm', 'is', null).order('date', { ascending: false }),
        supabase.from('grades').select('student_id, score, assessments!inner(max_score)').in('student_id', ids).not('score', 'is', null),
      ])
      // Latest CWPM per student
      const cwpmMap: Record<string, number> = {}
      readings?.forEach((r: any) => { if (!cwpmMap[r.student_id]) cwpmMap[r.student_id] = r.cwpm })
      // Average grade pct per student
      const gradeAcc: Record<string, { sum: number; count: number }> = {}
      grades?.forEach((g: any) => {
        const max = g.assessments?.max_score || 1
        const pct = (g.score / max) * 100
        if (!gradeAcc[g.student_id]) gradeAcc[g.student_id] = { sum: 0, count: 0 }
        gradeAcc[g.student_id].sum += pct
        gradeAcc[g.student_id].count++
      })
      const pts = students.filter(s => cwpmMap[s.id] != null && gradeAcc[s.id]).map(s => ({
        name: s.english_name, korean: s.korean_name,
        cwpm: cwpmMap[s.id], avg: gradeAcc[s.id].sum / gradeAcc[s.id].count,
        cls: s.english_class as EnglishClass,
      }))
      setPoints(pts)
  ...
  if (loading || loadingData) return <div className="p-12 text-center"><Loader2 size={20} className="animate-spin text-navy mx-auto" /></div>
  if (points.length < 2) return <div className="p-12 text-center text-text-tertiary text-[13px]">Need at least 2 students with both CWPM and grade data.</div>

  const W = 500, H = 340, PAD = 50
  const maxCwpm = Math.max(...points.map(p => p.cwpm)) * 1.1
  const minCwpm = Math.max(0, Math.min(...points.map(p => p.cwpm)) - 5)
  const maxAvg = Math.min(100, Math.max(...points.map(p => p.avg)) + 5)
  const minAvg = Math.max(0, Math.min(...points.map(p => p.avg)) - 5)
  const scaleX = (v: number) => PAD + ((v - minCwpm) / (maxCwpm - minCwpm)) * (W - PAD * 2)
  const scaleY = (v: number) => H - PAD - ((v - minAvg) / (maxAvg - minAvg)) * (H - PAD * 2)

  // Simple linear regression
  const n = points.length
  const sumX = points.reduce((s, p) => s + p.cwpm, 0)
  const sumY = points.reduce((s, p) => s + p.avg, 0)
  const sumXY = points.reduce((s, p) => s + p.cwpm * p.avg, 0)
  const sumX2 = points.reduce((s, p) => s + p.cwpm * p.cwpm, 0)
  const slope = (n * sumXY - sumX * sumY) / (n * sumX2 - sumX * sumX)
  const intercept = (sumY - slope * sumX) / n
  const rNum = n * sumXY - sumX * sumY
  const rDen = Math.sqrt((n * sumX2 - sumX * sumX) * (n * points.reduce((s, p) => s + p.avg * p.avg, 0) - sumY * sumY))
  const r = rDen !== 0 ? rNum / rDen : 0

  const lineX1 = minCwpm, lineY1 = slope * lineX1 + intercept
  const lineX2 = maxCwpm, lineY2 = slope * lineX2 + intercept
    ...
    <div className="px-10 py-6">
      <div className="bg-surface border border-border rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-[14px] font-semibold text-navy">Reading Fluency vs Grade Average</h3>
            <p className="text-[11px] text-text-secondary">Each dot is a student. Trend line shows correlation strength.</p>
          </div>
          <div className="text-right">
            <span className="text-[11px] text-text-secondary">Correlation: </span>
            <span className={`text-[14px] font-bold ${Math.abs(r) > 0.6 ? 'text-green-600' : Math.abs(r) > 0.3 ? 'text-amber-600' : 'text-text-tertiary'}`}>
              r = {r.toFixed(2)}
            </span>
            <span className="text-[10px] text-text-tertiary block">
              {Math.abs(r) > 0.7 ? 'Strong' : Math.abs(r) > 0.4 ? 'Moderate' : Math.abs(r) > 0.2 ? 'Weak' : 'Very weak'} correlation
            </span>
          </div>
        </div>
        <svg width={W} height={H} className="mx-auto">
          {/* Grid lines */}
          {[0, 25, 50, 75, 100].filter(v => v >= minAvg && v <= maxAvg).map(v => (
            <g key={`gy${v}`}>
              <line x1={PAD} x2={W - PAD} y1={scaleY(v)} y2={scaleY(v)} stroke="#e2e8f0" strokeDasharray="3,3" />
              <text x={PAD - 5} y={scaleY(v) + 3} textAnchor="end" fontSize={9} fill="#94a3b8">{v}%</text>
            </g>
          ))}
          {/* X axis labels */}
          {Array.from({ length: 5 }, (_, i) => Math.round(minCwpm + (i / 4) * (maxCwpm - minCwpm))).map(v => (
            <text key={`gx${v}`} x={scaleX(v)} y={H - PAD + 15} textAnchor="middle" fontSize={9} fill="#94a3b8">{v}</text>
          ))}
          {/* Axis labels */}
          ...
          {points.map((p, i) => (
            <g key={i}>
              <circle cx={scaleX(p.cwpm)} cy={scaleY(p.avg)} r={5} fill={classToColor(p.cls)} stroke={classToTextColor(p.cls)} strokeWidth={1.5} opacity={0.85} />
              <title>{p.name} ({p.korean}) -- CWPM: {Math.round(p.cwpm)}, Avg: {p.avg.toFixed(1)}%</title>
            </g>
          ))}
        </svg>
        {/* Legend */}
        <div className="flex items-center justify-center gap-3 mt-2 flex-wrap">
          {ENGLISH_CLASSES.map(cls => {
            const count = points.filter(p => p.cls === cls).length
            if (count === 0) return null
            return (
              <span key={cls} className="inline-flex items-center gap-1 text-[9px]">
                <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: classToColor(cls), border: `1.5px solid ${classToTextColor(cls)}` }} />
                {cls} ({count})
              </span>
            )
          })}
        ...
        {/* Insight */}
        <div className="mt-3 bg-blue-50 border border-blue-200 rounded-lg px-3 py-2 text-[10px] text-blue-800">
          {Math.abs(r) > 0.5
            ? `Strong positive relationship: students who read more fluently tend to score higher across all domains. This suggests reading fluency is a good predictor of overall academic performance.`
            : Math.abs(r) > 0.25
              ? `Moderate relationship between fluency and grades. Some students show strong grades despite lower fluency (may be strong writers/speakers) or high fluency with lower grades (may need comprehension support).`
              : `Weak correlation -- reading fluency and grade averages are relatively independent. Look for other factors driving grade differences.`
          }
        </div>
      </div>
    </div>
...
// ─── #32 Flexible Grouping Tool ───────────────────────────────────────

function FlexGroupingView({ students, loading, lang }: { students: any[]; loading: boolean; lang: string }) {
  const [mode, setMode] = useState<'homogeneous' | 'heterogeneous' | 'random'>('homogeneous')
  const [groupSize, setGroupSize] = useState(4)
  const [groups, setGroups] = useState<{ name: string; students: { id: string; name: string; cwpm: number | null }[] }[]>([])
  const [studentData, setStudentData] = useState<Record<string, number>>({})

  useEffect(() => {
    (async () => {
      const ids = students.map(s => s.id)
      if (ids.length === 0) return
      const { data } = await supabase.from('reading_assessments').select('student_id, cwpm').in('student_id', ids).not('cwpm', 'is', null).order('date', { ascending: false })
      const map: Record<string, number> = {}
      data?.forEach(r => { if (!map[r.student_id]) map[r.student_id] = r.cwpm })
      setStudentData(map)
    })()
  }, [students])

  const generateGroups = () => {
    const enriched = students.map(s => ({ id: s.id, name: s.english_name, cwpm: studentData[s.id] ?? null }))

    if (mode === 'random') {
      const shuffled = [...enriched].sort(() => Math.random() - 0.5)
      const result: typeof groups = []
      for (let i = 0; i < shuffled.length; i += groupSize) {
        result.push({ name: `Group ${result.length + 1}`, students: shuffled.slice(i, i + groupSize) })
      }
      setGroups(result)
      return
    }

    // Sort by CWPM (nulls at end)
    const sorted = [...enriched].sort((a, b) => {
      if (a.cwpm == null && b.cwpm == null) return 0
      if (a.cwpm == null) return 1
      if (b.cwpm == null) return -1
      return a.cwpm - b.cwpm
    })
    ...
    if (mode === 'homogeneous') {
      ...
      for (let i = 0; i < sorted.length; i += groupSize) {
        result.push({ name: `Group ${result.length + 1}`, students: sorted.slice(i, i + groupSize) })
      }
      setGroups(result)
    } else {
      // Heterogeneous: distribute evenly (snake draft)
      const numGroups = Math.ceil(sorted.length / groupSize)
      const result: typeof groups = Array.from({ length: numGroups }, (_, i) => ({ name: `Group ${i + 1}`, students: [] }))
      sorted.forEach((s, i) => {
        const round = Math.floor(i / numGroups)
        const idx = round % 2 === 0 ? i % numGroups : numGroups - 1 - (i % numGroups)
        result[idx].students.push(s)
      })
      setGroups(result.filter(g => g.students.length > 0))
    }
  }
  ...
  return (
    <div className="px-10 py-6 space-y-4">
      <div className="bg-surface border border-border rounded-xl p-4">
        <h3 className="text-[13px] font-semibold text-navy mb-3">Generate Student Groups</h3>
        <div className="flex items-end gap-4">
          <div>
            <label className="text-[10px] font-semibold text-text-secondary uppercase block mb-1">Grouping Strategy</label>
            <div className="flex gap-1">
              {[
                { id: 'homogeneous', label: 'Same Level', desc: 'Students with similar CWPM together' },
                { id: 'heterogeneous', label: 'Mixed Level', desc: 'Each group has high/mid/low readers' },
                { id: 'random', label: 'Random', desc: 'Random assignment' },
              ].map(m => (
                <button key={m.id} onClick={() => setMode(m.id as any)}
                  className={`px-3 py-1.5 rounded-lg text-[11px] font-medium ${mode === m.id ? 'bg-navy text-white' : 'bg-surface-alt text-text-secondary border border-border'}`}
                  title={m.desc}>
                  {m.label}
                </button>
              ))}
          ...
          <div>
            <label className="text-[10px] font-semibold text-text-secondary uppercase block mb-1">Group Size</label>
            <select value={groupSize} onChange={e => setGroupSize(+e.target.value)}
              className="px-3 py-1.5 border border-border rounded-lg text-[11px] bg-surface outline-none">
              {[2, 3, 4, 5, 6].map(n => <option key={n} value={n}>{n} students</option>)}
            </select>
          </div>
          <button onClick={generateGroups} className="px-4 py-2 rounded-lg text-[12px] font-medium bg-navy text-white hover:bg-navy-dark">
            Generate Groups
          </button>
        </div>
      </div>
      ...
      {groups.length > 0 && (
        <div className="grid grid-cols-3 gap-3">
          {groups.map(g => (
            <div key={g.name} className="bg-surface border border-border rounded-xl p-4">
              <h4 className="text-[12px] font-bold text-navy mb-2">{g.name}</h4>
              <div className="space-y-1">
                {g.students.map(s => (
                  <div key={s.id} className="flex items-center justify-between text-[11px]">
                    <span className="text-text-primary">{s.name}</span>
                    <span className={`font-medium ${s.cwpm != null ? 'text-navy' : 'text-text-tertiary'}`}>
                      {s.cwpm != null ? `${Math.round(s.cwpm)} cwpm` : '--'}
                    </span>
                  </div>
                ))}
              </div>
              {g.students.some(s => s.cwpm != null) && (
                <div className="mt-2 pt-2 border-t border-border/50 text-[9px] text-text-tertiary">
                  Avg: {Math.round(g.students.filter(s => s.cwpm != null).reduce((s, x) => s + (x.cwpm || 0), 0) / g.students.filter(s => s.cwpm != null).length)} cwpm
                  {' | '}Range: {Math.round(Math.min(...g.students.filter(s => s.cwpm != null).map(s => s.cwpm!)))}-{Math.round(Math.max(...g.students.filter(s => s.cwpm != null).map(s => s.cwpm!)))}
                </div>
              )}
            </div>
          ))}
