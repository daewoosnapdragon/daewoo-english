        ...
        {NAV_ITEMS.map((item, i) => {
          ...
          if (collapsed) {
            return (
              <button key={item.id} onClick={() => onNavigate(item.id!)} title={label}
                ...
                {isActive && <span className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-r-full bg-gold" />}
          ...
          return (
            <button key={item.id} onClick={() => onNavigate(item.id!)}
              className={`nav-icon-wiggle w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[12.5px] font-medium transition-all mb-0.5 relative ${isActive ? 'bg-gold/15 text-gold' : 'text-blue-200/70 hover:text-white hover:bg-white/5'}`}>
              {isActive && <span className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-r-full bg-gold" />}
