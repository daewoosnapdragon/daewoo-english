                      ...
                      {isExp && (
                        <div className="border-t border-border bg-surface-alt/30 px-4 py-2 space-y-1">
                              ...
                              <div key={std.code} className="flex items-start gap-2.5 py-1.5">
                                <button onClick={async () => {
                                  ...
                                  <div>
                                    ...
                                    {/* #28: Teaching guidance for this standard */}
                                    {(() => {
                                      const tips = getTeachingTip(std.code)
                                      if (!tips) return null
                                      return (
                                        <details className="mt-1">
                                          <summary className="text-[9px] text-indigo-600 cursor-pointer hover:underline">Teaching tip</summary>
                                          <div className="mt-1 text-[10px] text-indigo-800 bg-indigo-50 rounded px-2 py-1.5 leading-relaxed">{tips}</div>
                                        </details>
                                      )
                                    })()}
                                  </div>
                          ...
                          {(() => {
                            ...
                            const clusterAvg = avgs.length > 0 ? Math.round(avgs.reduce((a, b) => a + b, 0) / avgs.length) : null
                            const weakStds = cluster.codes.filter(c => stdAverages[c] != null && stdAverages[c] < t.approaching).map(c => ({ code: c, avg: Math.round(stdAverages[c]) }))
                            return (
                              <div className="border-t border-border bg-gradient-to-r from-blue-50/50 to-surface px-4 py-3">
                                <div className="flex items-center gap-4 mb-2">
                                  <div className="flex-1 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                                    <div className="h-full rounded-full bg-green-500 transition-all" style={{ width: `${pct}%` }} />
                                  </div>
                                  <span className="text-[10px] font-bold text-navy">{pct}% mastered</span>
                                </div>
                                <div className="flex gap-3 text-[9px] mb-2">
                                  <span className="text-green-600">{masteredCount} mastered</span>
                                  <span className="text-blue-600">{ipCount} in progress</span>
                                  <span className="text-text-tertiary">{nsCount} not started</span>
                                  {clusterAvg != null && <span className="ml-auto font-bold text-navy">Cluster avg: {clusterAvg}%</span>}
                                </div>
                                {weakStds.length > 0 && (
                                  <div className="text-[10px] text-red-700 bg-red-50 rounded px-2 py-1.5 mb-1.5">
                                    <span className="font-semibold">Focus areas: </span>
                                    {weakStds.map(s => `${s.code} (${s.avg}%)`).join(', ')}
                                  </div>
                                )}
