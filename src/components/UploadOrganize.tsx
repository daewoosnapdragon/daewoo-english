  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {/* Needs Attention */}
        {(untagged.length > 0 || noCategory.length > 0 || packets.length > 0) && (
          <div className="bg-cyber-surface border border-cyber-border rounded-none p-3">
            ...
            <div className="flex flex-wrap gap-2">
              {untagged.length > 0 && (
                <span className="text-[10px] bg-cyber-surface text-cyber-fg px-2 py-1 rounded-none font-medium">{untagged.length} not AI-tagged</span>
              )}
              {noCategory.length > 0 && (
                <span className="text-[10px] bg-cyber-surface text-cyber-fg px-2 py-1 rounded-none font-medium">{noCategory.length} no category</span>
              )}
              {packets.length > 0 && (
                <span className="text-[10px] bg-cyber-surface text-cyber-fg px-2 py-1 rounded-none font-medium">{packets.length} multi-page packets</span>
              )}
            </div>
            {untagged.length > 0 && (
              <AutoTagAllButton resources={untagged} onComplete={() => { fetchRecent(); onResourcesChanged(); }} />
            )}
          </div>
        )}
