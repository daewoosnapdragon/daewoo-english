...
function AcademicHistoryTab({ studentId, lang }: { studentId: string; lang: 'en' | 'ko' }) {
  ...
  const DOMAIN_LABELS: Record<string, Record<string, string>> = {
    reading: { en: 'Reading', ko: '읽기' }, phonics: { en: 'Phonics & Foundational Skills', ko: '파닉스' },
    writing: { en: 'Writing', ko: '쓰기' }, speaking: { en: 'Speaking & Listening', ko: '말하기/듣기' },
...
function buildStudentPDFHtml(student: Student, data: {
  ...
  const { grades, semesterGrades, readingRecords, behaviorLogs, attendanceRecords, comments, scaffolds = [], goals = [], assessments = [] } = data
  const date = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })

  // Build assessment lookup for standards info
  const assessmentMap: Record<string, any> = {}
  assessments.forEach(a => { assessmentMap[a.id] = a })

  // Semester grades: group by semester, then by domain
  const semGrouped: Record<string, { name: string; year: string; type: string; domains: Record<string, any> }> = {}
  semesterGrades.forEach((sg: any) => {
    const semName = sg.semesters?.name || '—'
    const semKey = sg.semester_id || semName
    if (!semGrouped[semKey]) semGrouped[semKey] = { name: semName, year: sg.semesters?.academic_year || '', type: sg.semesters?.type || '', domains: {} }
    semGrouped[semKey].domains[sg.domain] = sg
  })

  const semesterHTML = Object.values(semGrouped).length > 0
    ? Object.values(semGrouped).map(sem => {
      const domainRows = Object.entries(sem.domains).filter(([d]) => d !== 'overall').map(([d, sg]: [string, any]) =>
        `<tr><td style="padding:5px 8px">${domainLabel(d)}</td><td style="padding:5px 8px;text-align:center;font-weight:600">${sg.final_grade != null ? sg.final_grade.toFixed(1) + '%' : sg.calculated_grade != null ? sg.calculated_grade.toFixed(1) + '%' : '—'}</td><td style="padding:5px 8px;text-align:center">${sg.letter_grade || '—'}</td><td style="padding:5px 8px;text-align:center">${sg.behavior_grade || '—'}</td></tr>`
      ).join('')
      const overall = sem.domains['overall']
      return `<h3>${sem.name} ${sem.year ? '(' + sem.year + ')' : ''}</h3>
        <table><tr><th>Domain</th><th>Score</th><th>Letter</th><th>Behavior</th></tr>${domainRows}
        ${overall ? `<tr style="background:#EEF2FF;font-weight:700"><td style="padding:5px 8px">Overall</td><td style="padding:5px 8px;text-align:center">${overall.final_grade != null ? overall.final_grade.toFixed(1) + '%' : '—'}</td><td style="padding:5px 8px;text-align:center">${overall.letter_grade || '—'}</td><td style="padding:5px 8px;text-align:center">${overall.behavior_grade || '—'}</td></tr>` : ''}
        </table>`
    }).join('')
    : '<p class="empty">No semester grades recorded.</p>'

  // Standards with descriptions — collect all tagged standards
  const allStds: Record<string, { code: string; desc: string; assessments: string[] }> = {}
  grades.forEach((g: any) => {
    const a = g.assessments || assessmentMap[g.assessment_id]
    if (!a?.standards) return
    a.standards.forEach((st: any) => {
      if (!allStds[st.code]) allStds[st.code] = { code: st.code, desc: st.description || '', assessments: [] }
      if (!allStds[st.code].assessments.includes(a.name)) allStds[st.code].assessments.push(a.name)
    })
  })
  const standardsHTML = Object.keys(allStds).length > 0
    ? `<table><tr><th>Standard</th><th>Description</th><th>Assessed In</th></tr>${Object.values(allStds).sort((a, b) => a.code.localeCompare(b.code)).map(s =>
      `<tr><td style="padding:5px 8px;font-weight:700;white-space:nowrap">${s.code}</td><td style="padding:5px 8px;font-size:9px">${s.desc}</td><td style="padding:5px 8px;font-size:9px;color:#64748b">${s.assessments.join(', ')}</td></tr>`
    ).join('')}</table>`
    : '<p class="empty">No standards tagged yet.</p>'

  // Detailed behavior logs
  const behaviorHTML = behaviorLogs.length > 0 ? `<table>
  <tr><th>Date</th><th>Type</th><th>Note</th>${behaviorLogs.some(b => b.antecedents?.length) ? '<th>ABC Details</th>' : ''}</tr>
  ${behaviorLogs.map((b: any) => {
    const typeColor = b.type === 'positive' ? '#16a34a' : b.type === 'concern' || b.type === 'negative' ? '#dc2626' : b.type === 'parent_contact' ? '#7c3aed' : b.type === 'intervention' ? '#2563eb' : '#64748b'
    const typeLabel = b.type === 'parent_contact' ? 'Parent Contact' : b.type.charAt(0).toUpperCase() + b.type.slice(1)
    const abcDetail = b.antecedents?.length || b.behaviors?.length || b.consequences?.length
      ? `A: ${(b.antecedents || []).join(', ')} → B: ${(b.behaviors || []).join(', ')} → C: ${(b.consequences || []).join(', ')}${b.intensity ? ` (Intensity: ${b.intensity}/5)` : ''}`
      : ''
    return `<tr><td style="padding:5px 8px;white-space:nowrap">${b.date}${b.time ? ' ' + b.time : ''}</td><td style="padding:5px 8px;color:${typeColor};font-weight:600;font-size:10px">${typeLabel}${b.is_flagged ? ' 🚩' : ''}</td><td style="padding:5px 8px">${b.note || '—'}</td>${behaviorLogs.some(bl => bl.antecedents?.length) ? `<td style="padding:5px 8px;font-size:9px;color:#64748b">${abcDetail}</td>` : ''}</tr>`
  }).join('')}
</table>` : '<p class="empty">No behavior logs recorded.</p>'

  // Comments — show all with semester context
  const commentsHTML = comments.length > 0
    ? comments.map((c: any) => `<div class="note"><strong>${c.semesters?.name || '—'} ${c.semesters?.academic_year ? '(' + c.semesters.academic_year + ')' : ''}:</strong> ${c.text || c.comment || '—'}</div>`).join('')
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; font-size: 11px; color: #1e293b; padding: 40px; max-width: 850px; margin: 0 auto; }
  h1 { font-size: 22px; color: #647FBC; margin-bottom: 4px; }
  h2 { font-size: 14px; color: #647FBC; border-bottom: 2px solid #647FBC; padding-bottom: 4px; margin: 24px 0 10px; page-break-after: avoid; }
  h3 { font-size: 12px; color: #475569; margin: 12px 0 6px; }
  .header { border-bottom: 3px solid #647FBC; padding-bottom: 12px; margin-bottom: 20px; }
  .meta { color: #64748b; font-size: 12px; }
  .badge { display: inline-block; padding: 2px 8px; border-radius: 10px; font-size: 10px; font-weight: 700; background: #dbeafe; color: #647FBC; }
...
<h2>Standards Covered (${Object.keys(allStds).length})</h2>
${standardsHTML}
...
<div>${goals.map((g: any) => `<div class="note"><span style="${g.completed_at ? 'text-decoration:line-through;color:#94a3b8' : ''}">${g.goal_text}</span>${g.completed_at ? ` <span style="color:#059669;font-size:9px">Done ${new Date(g.completed_at).toLocaleDateString()}</span>` : ''}</div>`).join('')}
</div>` : ''}

<h2>Behavior Log (${behaviorLogs.length} entries)</h2>
${behaviorHTML}
...
<p style="font-size:8px;color:#94a3b8;margin-top:32px;border-top:1px solid #e2e8f0;padding-top:8px">Daewoo Elementary English Program · Student Portfolio · Generated ${date}</p>
